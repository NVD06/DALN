...
@section('content')
    <div class="container">
        <div class="card shadow-lg p-4">
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <h4 class="text-muted">👨‍🏫 Mentor:
                        <strong>{{ $course->mentor ? $course->mentor->name : 'Không xác định' }}</strong></h4>
                    <p><strong>📖 Mô tả:</strong> {{ $course->description }}</p>
                    <p><strong>📚 Số chương:</strong> {{ $course->modules->count() }}</p>
                    <p><strong>💰 Giá:</strong> <span
                            class="badge bg-danger fs-5">{{ number_format($course->price, 0, ',', '.') }} VNĐ</span></p>
                ...
                @endif --}}
                    @if ($course->isRegistered(Auth::id()))
                        <p><strong>⏳ Tiến trình:</strong> <span
                                class="badge bg-success">{{ $course->getProgress(Auth::id()) }}%</span></p>
                        <a href="{{ route('courses.learn', $course->id) }}" class="btn btn-success"><i
                                class="fas fa-play"></i> Vào học</a>
                    @else
                        <a href="{{ Auth::check() ? route('courses.payment', $course->id) : route('login') }}"
                ...
                <ul class="list-group">
                    @foreach ($course->students as $student)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            {{ $student->name }}
                            <a href="{{ route('messages.show', ['course' => $course->id, 'receiver' => $student->id]) }}"
                                class="btn btn-primary btn-sm"><i class="fas fa-comments"></i> Nhắn tin</a>
                        </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="mt-5">
            ...
            @if ($course->reviews->count() > 0)
                <div class="card shadow-sm">
                    <div class="card-body">
                        <h4>⭐ Đánh giá trung bình: <span
                                class="text-warning">{{ number_format($course->reviews->avg('rating'), 1) }}/5</span></h4>
                        <ul class="list-group">
        ...
        <a href="{{ route('courses.index') }}" class="btn btn-secondary mt-3"><i class="fas fa-arrow-left"></i> Quay lại
